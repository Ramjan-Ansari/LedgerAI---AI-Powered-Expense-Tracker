...
import { generateBudgetAlert, generateMonthlyInsight, generateSavingsTips } from '../utils/gimini.js';
...
const buildMonthlyInsight = async (userId) => {
    ...
    return { content, periodEnd, periodStart };
...
const buildSavingsTips = async (userId) => {
    ...
    const content = await generateSavingsTips({
        ...
        monthlyIcome: parseFloat(incomeResult.rows[0].income),
        currency,
    });
...
const buildBudgetAlert = async (userId, categoryId) => {
    ...
    const content = await generateBudgetAlert({
        ...
        spendAmount: parseFloat(b.spend),
        daysIntoPeriod,
        totalPeriodDays,
        currency,
    });
...
export { getUserCurrency, getInsights, buildBudgetAlert, buildMonthlyInsight, buildSavingsTips, generateInsight };