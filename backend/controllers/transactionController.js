...
import { analyzeTransactionList } from '../utils/gimini.js';
...
const createTransaction = async (req, res) => {
    ...
    const note = notes ?? req.body.note ?? null;

    if (amount === undefined || !type || !transactionDate) {
    ...
    try {
        const result = await pool.query(
            `INSERT INTO transactions (user_id, category_id, amount, type, description, note, transaction_date)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            RETURNING *`,
            [req.userId, categoryId || null, amount, type, description || null, note, transactionDate]
        );
...
const updateTransaction = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            RETURNING *`,
            [categoryId, amount, type, description, note, transactionDate, id, req.userId]
        );
