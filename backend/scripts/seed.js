    ...
    for(let monthsAgo = 11; monthsAgo >= 0; monthsAgo--) {
        ...
        for (let day = 1; day <= monthLastDay; day++){
            ...
            if(!isWeekend && rng() < 0.5){
                add(day, 'Food & Dining', rangeFloat(28, 75), 'expense', pick(['Dinner out', 'Restaurant', 'Branch']));
            }
            if(!isWeekend && rng() < 0.4){
                add(day, 'Transportation', rangeFloat(2.5, 6), 'expense', pick(['Sub Way', 'Parking']));
            }
            if(rng() < 0.15){
        ...
        for( let day = 1; day <= monthLastDay; day++){
            const d = new Date(monthStart.getFullYear(), monthStart.getMonth(), day);
            if(d.getDay() === 0) {
                add(day, 'Groceries', rangeFloat(55, 120), 'expense', pick(['Whole Foods', "Trader Joe's", "Weekly"]))
            }
        }

        //weekly gas
        for(let day = 4; day<= monthLastDay; day++){
        ...
        if([10, 6, 2].includes(monthsAgo)) {
            add(rangeInt(10, 20), 'Healthcare', rangeFloat(40, 135), 'expense', pick(['Doctor visit']));
        }
        if(monthsAgo % 2 === 0) {
            add(rangeInt(8, 14), 'Personal Care', rangeFloat(35, 55), 'expense', pick(['Haircut']));
        }
        if([11, 7, 3].includes(monthsAgo)) {
            add(rangeInt(15, 22), 'Travel', rangeFloat(180, 380), 'expense', pick(['Weekend trip']));
        }
    }
    return txns;
}
